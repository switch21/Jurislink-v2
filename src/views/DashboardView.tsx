...
import { queryClient, STATUS_COLORS, PRIORITY_COLORS, CRIT_COLORS, CHART_COLORS, TASK_STATUS_MAP } from './constants'
import { fmtDate, fmtDateTime, fmtMoney, fmtFileSize, initials, relativeTime, fmtDuration, taskStatusColor, taskStatusLabel, statusLabel, typeLabel, eventTypeLabel } from './helpers'
...
export function DashboardView() {
  ...
  const statusChartData = Object.entries(stats.casesByStatus || {}).map(([name, value]) => ({ name: statusLabel(name), value })).filter(d => d.value > 0)
  const typeChartData = Object.entries(stats.casesByType || {}).map(([name, value]) => ({ name: typeLabel(name), value })).filter(d => d.value > 0)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className={cn('border-l-4', todayEvtCount > 0 ? 'border-l-[var(--primary)]' : urgencyCount > 0 ? 'border-l-[#DC2626]' : 'border-l-[#059669]')}><CardHeader className="pb-3"><CardTitle className="text-sm font-semibold flex items-center gap-2">{todayEvtCount > 0 ? <><Calendar className="size-4 text-[var(--primary)]" />Événements aujourd'hui ({todayEvtCount})</> : urgencyCount > 0 ? <><AlertOctagon className="size-4 text-[var(--danger)]" />Urgences ({urgencyCount})</> : <><CheckCircle2 className="size-4 text-[var(--success)]" />Aucune urgence</>}</CardTitle></CardHeader><CardContent className="p-4 pt-0"><div className="space-y-2 max-h-64 overflow-y-auto">
            {todayEvtCount > 0 ? (stats.todayEvents || []).map((e: { id: string; title: string; startTime: string; eventType: string; criticality: string; caseReference: string | null; assignments: Array<{ userName: string }> }) => (
              <div key={e.id} className="flex items-center gap-3 p-2 rounded-lg bg-[var(--primary-light)] cursor-pointer" onClick={() => setCurrentView('calendar')}><span className={cn('w-1 h-8 rounded-full shrink-0', CRIT_COLORS[e.criticality] || CRIT_COLORS.normal)} /><div className="min-w-0 flex-1"><p className="text-sm font-medium">{e.title}</p><p className="text-xs text-[var(--text-secondary)]">{fmtDateTime(e.startTime)}{e.caseReference ? ` • ${e.caseReference}` : ''}</p><p className="text-[10px] text-[var(--text-muted)] mt-0.5">{e.assignments.map(a => a.userName).join(', ') || 'Non assigné'}</p></div><Badge variant="outline" className="text-[10px] shrink-0">{eventTypeLabel(e.eventType)}</Badge></div>
            )) : urgencyCount > 0 ? <>
          ...
          {/* Upcoming Events */}
          <Card><CardHeader className="pb-3"><CardTitle className="text-sm font-semibold flex items-center gap-2"><Calendar className="size-4 text-[var(--accent)]" />Prochains événements (7j)</CardTitle></CardHeader><CardContent className="p-4 pt-0"><div className="space-y-2 max-h-64 overflow-y-auto">{(stats.upcomingEventsEnhanced || []).length === 0 ? <p className="text-xs text-[var(--text-muted)] py-4 text-center">Aucun événement à venir</p> : (stats.upcomingEventsEnhanced || []).map(e => (<div key={e.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-[var(--border-light)] cursor-pointer" onClick={() => setCurrentView('calendar')}><span className={cn('w-1 h-8 rounded-full shrink-0', CRIT_COLORS[e.criticality] || CRIT_COLORS.normal)} /><div className="min-w-0 flex-1"><p className="text-sm font-medium">{e.title}</p><p className="text-xs text-[var(--text-secondary)]">{fmtDateTime(e.startTime)}{e.caseReference ? ` • ${e.caseReference}` : ''}</p><p className="text-xs text-[var(--text-muted)] mt-0.5">{e.assignments.map(a => a.userName).join(', ')}</p></div><Badge variant="outline" className="text-[10px] shrink-0">{eventTypeLabel(e.eventType)}</Badge></div>))}</div></CardContent></Card>
      </div>
          ...
          <CardContent className="p-4 pt-0"><div className="space-y-2 max-h-64 overflow-y-auto">
            {(stats.casesWithoutDeadlines || []).length === 0 ? <p className="text-xs text-[var(--success)] py-4 text-center">Tous les dossiers actifs ont des échéances</p> : (stats.casesWithoutDeadlines || []).map(c => (
              <div key={c.id} className="flex items-center gap-3 p-2 rounded-lg bg-[var(--primary-light)] hover:bg-jl-blue-light cursor-pointer" onClick={() => setCurrentView('cases')}>
                ...
                <Badge variant="outline" className="text-[10px] shrink-0 border-[#6366F1] text-[#6366F1]">{statusLabel(c.status)}</Badge>
              </div>
            ))}
          </div></CardContent>
        </Card>
