  ...
  return (<div className='p-6 space-y-6'>
    ...
    <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
      ...
      <Card className='p-4'><CardTitle className='text-sm font-semibold mb-3'>Cabinets récents</CardTitle>
        <div className='space-y-3'>{(Array.isArray(data.recentTenants) ? data.recentTenants : []).slice(0, 5).map((tn) => { const userCount = tn._count?.users ?? 0; return (<div key={tn.id} className='flex items-center justify-between'><div><p className='text-sm font-medium text-jl-primary'>{tn.name}</p><p className='text-xs text-jl-muted'>{userCount} utilisateur{userCount > 1 ? 's' : ''} · {fmtDate(tn.createdAt)}</p></div>{tn.subscription?.plan && <Badge className='bg-jl-blue-light text-jl-blue text-[10px]'>{tn.subscription.plan.name}</Badge>}</div>) })}</div>
      </Card>
    </div>
  ...
  return (<div className='p-6 space-y-6'>
    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
      ...
      <Card><CardHeader className='pb-3'><CardTitle className='text-sm font-semibold'>Top 3 cabinets par dossiers</CardTitle></CardHeader><CardContent className='space-y-3'>
        {topByCases.map((tn, i) => (<div key={tn.id} className='flex items-center gap-3 p-3 rounded-lg bg-jl-page'><div className={cn('flex items-center justify-center size-8 rounded-full text-sm font-bold', i === 0 ? 'bg-jl-gold text-white' : i === 1 ? 'bg-jl-page text-white' : 'bg-jl-gold text-white')}>{i + 1}</div><div className='flex-1 min-w-0'><p className='text-sm font-medium text-jl-primary truncate'>{tn.name}</p><p className='text-[10px] text-jl-muted'>{tn._count?.users ?? 0} utilisateurs · {tn.subscription?.plan?.name || tn.plan}</p></div><div className='text-right'><p className='text-lg font-bold text-jl-blue'>{tn._count?.cases ?? 0}</p><p className='text-[10px] text-jl-muted'>dossiers</p></div></div>))}
