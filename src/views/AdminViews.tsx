...
import { fmtDate, fmtDateTime, fmtMoney, fmtFileSize, initials, relativeTime, fmtDuration, taskStatusColor, taskStatusLabel, roleLabel } from './helpers'
...
export function AdminDashboardView() {
  ...
  const kpis = [
    { label: t('admin.activeCab2'), value: data.activeTenants, icon: BuildingIcon, color: 'text-jl-blue' },
    { label: t('admin.activeUsers2'), value: data.activeUsers, icon: UsersRound, color: 'text-[var(--success)]' },
    { label: t('admin.activeCases2'), value: data.activeCases, icon: Briefcase, color: 'text-jl-gold' },
    { label: t('admin.totalClients2'), value: data.totalClients, icon: Users, color: 'text-[#7C3AED]' },
    { label: t('admin.totalRevenue2'), value: fmtMoney(data.totalRevenue), icon: TrendingUp, color: 'text-jl-blue' },
    { label: t('admin.monthRevenue2'), value: fmtMoney(data.thisMonthRevenue), icon: DollarSign, color: 'text-[var(--success)]' },
  ]
  return (<div className='p-6 space-y-6'>
    ...
    <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
      ...
      <Card className='p-4'><CardTitle className='text-sm font-semibold mb-3'>Cabinets récents</CardTitle>
        <div className='space-y-3'>{(Array.isArray(data.recentTenants) ? data.recentTenants : []).slice(0, 5).map((tn) => (<div key={tn.id} className='flex items-center justify-between'><div><p className='text-sm font-medium text-jl-primary'>{t.name}</p><p className='text-xs text-jl-muted'>{t._count.users} utilisateur{tn._count.users > 1 ? 's' : ''} · {fmtDate(tn.createdAt)}</p></div>{t.subscription?.plan && <Badge className='bg-jl-blue-light text-jl-blue text-[10px]'>{tn.subscription.plan.name}</Badge>}</div>))}</div>
      </Card>
    </div>
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
      ...
      <Card className='p-4'><CardTitle className='text-sm font-semibold mb-4'>Distribution par rôle</CardTitle>
        <div className='space-y-3'>{(data.usersByRole || []).map(r => (<div key={r.role}><div className='flex justify-between text-xs mb-1'><span className='text-jl-secondary'>{roleLabel(r.role)}</span><span className='text-jl-secondary'>{r._count.id}</span></div><div className='h-2 bg-jl-page rounded-full overflow-hidden'><div className='h-full bg-jl-blue rounded-full' style={{ width: `${(r._count.id / maxRole) * 100}%` }} /></div></div>))}</div>
      </Card>
    </div>
  </div>)
}
...
export function TenantRow({ t, subDaysLeft, openSubDialog, openEdit, delMut }: { t: AdminTenant; subDaysLeft: (t: AdminTenant) => number | null; openSubDialog: (t: AdminTenant) => void; openEdit: (t: AdminTenant) => void; delMut: { mutate: (id: string) => void } }) {
  const tn = t; const dl = subDaysLeft(tn)
  return (<TableRow className='cursor-pointer hover:bg-jl-page'><TableCell><div className='flex items-center gap-2'><div className={cn('size-2 rounded-full shrink-0', tn.isActive ? 'bg-[var(--success)]' : 'bg-jl-page')} /><div><span className='font-medium text-jl-primary'>{tn.name}</span>{tn.city && <p className='text-[10px] text-jl-muted'>{tn.city}{tn.country ? `, ${tn.country}` : ''}</p>}</div></div></TableCell><TableCell className='hidden sm:table-cell'><div className='flex flex-col gap-1'><div className='flex items-center gap-1.5'><Badge className='bg-jl-blue-light text-jl-blue text-[10px]'>{tn.subscription?.plan?.name || tn.plan}</Badge>{dl !== null && <span className={cn('text-[10px] font-medium', dl <= 0 ? 'text-[var(--danger)]' : dl <= 15 ? 'text-[var(--accent)]' : 'text-[var(--success)]')}>{dl <= 0 ? t('admin.expired') : dl + 'j restants'}</span>}</div>{tn.subscription?.currentPeriodEnd && <p className='text-[10px] text-jl-muted'>Fin : {new Date(tn.subscription.currentPeriodEnd).toLocaleDateString('fr-FR')}</p>}</div></TableCell><TableCell><div className='flex items-center gap-1.5'><Users className='size-3 text-jl-muted' /><span className='text-sm font-medium'>{tn._count?.users ?? 0}</span></div></TableCell><TableCell><div className='flex items-center gap-1.5'><Briefcase className='size-3 text-jl-muted' /><span className='text-sm font-medium'>{tn._count?.cases ?? 0}</span></div></TableCell><TableCell className='hidden md:table-cell text-sm text-jl-secondary'>{tn._count?.clients ?? 0}</TableCell><TableCell className='hidden lg:table-cell text-sm text-jl-secondary'>{tn._count?.invoices ?? 0}</TableCell><TableCell className='hidden lg:table-cell text-xs text-jl-secondary'>{fmtDate(tn.createdAt)}</TableCell><TableCell><div className='flex items-center gap-0.5'><Button variant='ghost' size='icon' className='size-7 text-jl-blue hover:text-jl-blue hover:bg-jl-blue-light' onClick={() => openSubDialog(tn)} title={t('admin.manageSub')}><CreditCard className='size-3.5' /></Button><Button variant='ghost' size='icon' className='size-7' onClick={() => openEdit(tn)}><Edit className='size-3.5' /></Button><Button variant='ghost' size='icon' className='size-7 text-[var(--danger)]' onClick={() => delMut.mutate(tn.id)}><Trash2 className='size-3.5' /></Button></div></TableCell></TableRow>)
}
...
export function AdminCabinsView() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-tenants'] }); toast.success(editing ? t('admin.cabinetUpdated2') : t('admin.cabinetCreated2')); setDialogOpen(false) },
    onError: () => toast.error(t('admin.errorSave'))
  })
  const delMut = useMutation({
    mutationFn: (id: string) => fetch(`/api/tenants/${id}`, { method: 'DELETE' }).then(r => { if (!r.ok) throw new Error(); }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-tenants'] }); toast.success(t('admin.cabinetDeleted2')) },
  ...
  const subMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-tenants'] }); qc.invalidateQueries({ queryKey: ['admin-plans-subs'] }); toast.success(t('settings.subUpdated2')); setSubDialogOpen(false) },
  ...
  const kpis = [
    { label: t('admin.activeCab2'), value: totalActive, icon: BuildingIcon, color: 'text-jl-blue', bg: 'bg-jl-blue-light', ring: 'ring-[#1E5A8A]/10' },
  ...
  return (<div className='p-6 space-y-6'>
    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
      ...
      <Card><CardHeader className='pb-3'><CardTitle className='text-sm font-semibold'>Top 3 cabinets par dossiers</CardTitle></CardHeader><CardContent className='space-y-3'>
        {topByCases.map((tn, i) => (<div key={tn.id} className='flex items-center gap-3 p-3 rounded-lg bg-jl-page'><div className={cn('flex items-center justify-center size-8 rounded-full text-sm font-bold', i === 0 ? 'bg-jl-gold text-white' : i === 1 ? 'bg-jl-page text-white' : 'bg-jl-gold text-white')}>{i + 1}</div><div className='flex-1 min-w-0'><p className='text-sm font-medium text-jl-primary truncate'>{t.name}</p><p className='text-[10px] text-jl-muted'>{tn._count?.users ?? 0} utilisateurs · {tn.subscription?.plan?.name || tn.plan}</p></div><div className='text-right'><p className='text-lg font-bold text-jl-blue'>{tn._count?.cases ?? 0}</p><p className='text-[10px] text-jl-muted'>dossiers</p></div></div>))}
    ...
    <Card><CardContent className='p-0'><div className='max-h-[480px] overflow-y-auto'><Table><TableHeader><TableRow><TableHead>Nom</TableHead><TableHead className='hidden sm:table-cell'>Abonnement</TableHead><TableHead>Utilisateurs</TableHead><TableHead>Dossiers</TableHead><TableHead className='hidden md:table-cell'>Clients</TableHead><TableHead className='hidden lg:table-cell'>Factures</TableHead><TableHead className='hidden lg:table-cell'>Créé le</TableHead><TableHead className='w-[100px]'>Actions</TableHead></TableRow></TableHeader><TableBody>
      ...
      tenants.map((tn) => (<TenantRow key={tn.id} t={tn} subDaysLeft={subDaysLeft} openSubDialog={openSubDialog} openEdit={openEdit} delMut={delMut} />))}
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className='max-w-lg max-h-[90vh] overflow-y-auto'><DialogHeader><DialogTitle>{editing ? 'Modifier le cabinet' : 'Nouveau cabinet'}</DialogTitle></DialogHeader>
      <div className='space-y-3'>
        ...
        <label className='flex items-center gap-2 cursor-pointer'><Switch checked={form.isActive} onCheckedChange={v => setForm({ ...form, isActive: v })} /><span className='text-sm'>{t('common.active')}</span></label>
      </div>
...
export function AdminUsersView() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-users'] }); toast.success(editing ? t('common.userUpdated') : t('common.userCreated')); setDialogOpen(false) },
    onError: () => toast.error('Erreur lors de la sauvegarde')
  })
  const delMut = useMutation({
    mutationFn: (id: string) => fetch(`/api/users/${id}`, { method: 'DELETE' }).then(r => { if (!r.ok) throw new Error(); }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-users'] }); toast.success(t('common.deleteSuccess')) },
  ...
  const toggleMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-users'] }); toast.success(t('common.statusUpdated')) },
  ...
  const adminChangePw = useMutation({
    ...
    onSuccess: () => { toast.success(t('common.passwordChanged')); setPwDialogOpen(false); setPwForm({ newPassword: '', confirmPassword: '' }); setPwTarget(null) },
  ...
  return (<div className='p-6 space-y-4'>
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className='max-w-lg max-h-[90vh] overflow-y-auto'><DialogHeader><DialogTitle>{editing ? 'Modifier l\'utilisateur' : 'Nouvel utilisateur'}</DialogTitle></DialogHeader>
      <div className='space-y-3'>
        ...
        <div><Label>Cabinet</Label><Select value={form.tenantId} onValueChange={v => setForm({ ...form, tenantId: v })}><SelectTrigger><SelectValue placeholder='Sélectionner…' /></SelectTrigger><SelectContent><SelectItem value=''>Aucun (root_admin)</SelectItem>{(tenantsData || []).map((tn) => <SelectItem key={tn.id} value={tn.id}>{tn.name}</SelectItem>)}</SelectContent></Select></div>
    ...
    <Dialog open={pwDialogOpen} onOpenChange={setPwDialogOpen}><DialogContent className='max-w-sm'><DialogHeader><DialogTitle>Modifier le mot de passe</DialogTitle><DialogDescription>Pour : <span className='font-semibold'>{pwTarget?.fullName}</span></DialogDescription></DialogHeader>
      <div className='space-y-3'>
        <div><Label>Nouveau mot de passe *</Label><Input type='password' value={pwForm.newPassword} onChange={e => setPwForm({ ...pwForm, newPassword: e.target.value })} placeholder={t('settings.minChars')} /></div>
