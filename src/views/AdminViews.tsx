...
export function AdminUsersView() {
  ...
  const forceLogoutMut = useMutation({
    ...
    onSuccess: (d) => { toast.success(d.message || 'Utilisateur déconnecté') },
    onError: (e: Error) => toast.error(e.message),
  })
  const forceLogoutAllMut = useMutation({
    mutationFn: () => fetch('/api/users/force-logout-all', { method: 'POST' }).then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error || 'Erreur'); return d }),
    onSuccess: (d) => { toast.success(d.message || 'Tous les utilisateurs déconnectés') },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  return (<div className='p-6 space-y-4'>
    <div className='flex items-center justify-between flex-wrap gap-3'>
      ...
      <div className='flex items-center gap-2'>
        <Button variant='outline' className='text-[var(--danger)] border-[var(--danger)] hover:bg-[var(--danger)] hover:text-white' disabled={forceLogoutAllMut.isPending} onClick={() => forceLogoutAllMut.mutate()}>{forceLogoutAllMut.isPending ? <RefreshCw className='size-4 animate-spin' /> : <Unplug className='size-4 mr-2' />}Déconnecter tous</Button>
    ...
    <div className='flex items-center gap-3 flex-wrap'>
      <div className='relative flex-1 min-w-[200px] max-w-sm'><Search className='absolute left-3 top-1/2 -translate-y-1/2 size-4 text-jl-muted' /><Input placeholder='Rechercher…' value={search} onChange={e => setSearch(e.target.value)} className='pl-9 h-9' /></div>
      <Select value={roleFilter} onValueChange={v => setRoleFilter(v)}><SelectTrigger className='w-[160px] h-9'><SelectValue placeholder='Rôle' /></SelectTrigger><SelectContent>{Object.entries(ROLE_LABELS).map(([k]) => <SelectItem key={k} value={k}>{roleLabel(k)}</SelectItem>)}</SelectContent></Select>
      <label className='flex items-center gap-2 text-sm text-jl-secondary cursor-pointer'><Switch checked={showInactive} onCheckedChange={setShowInactive} /><span>Voir inactifs</span></label>
    </div>
    {isLoading ? <div className='space-y-2'>{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className='h-12' />)}</div> :
