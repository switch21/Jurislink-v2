...
export function LoginPage() {
  ...
  const handleCabinetSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!email) { toast.error(t('login.errorFields')); return }
    if (!password) { toast.error(t('login.errorFields')); return }
    setLoading(true)
    try {
      const res = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) })
      const data = await res.json()
      if (!res.ok) { toast.error(data.error || t('login.errorInvalid')); return }
      if (data.mfaRequired) {
  ...
  const handleMfaSubmit = async (e: React.FormEvent) => {
    ...
    if (mfaCode.length !== 6) { toast.error(t('login.errorFields')); return }
    if (mfaTimeLeft <= 0) { toast.error(t('login.errorInvalid')); resetMfa() }
    setMfaLoading(true)
    try {
  ...
  const handlePortalSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!portalEmail) { toast.error(t('login.errorFields')); return }
    if (!portalPassword) { toast.error(t('login.errorFields')); return }
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { toast.error(data.error || t('portal.loginError')); return }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg-page)] p-4 transition-colors duration-300">
      <div className="w-full max-w-md">
            ...
            {tab === 'cabinet' && <>
              {!mfaPending ? <>
                ...
                <CardHeader className="text-center pb-2 pt-8">
                  ...
                  <CardTitle className="text-base font-semibold">{t('login.mfaVerify')}</CardTitle>
        ...
        <div className="text-center mt-3">
          <button type="button" onClick={() => useAppStore.getState().setCurrentView('pricing')} className="text-xs text-[var(--primary)] hover:underline font-medium">{t('login.viewPricing')}</button>
        </div>
      </div>
    </div>
  )
