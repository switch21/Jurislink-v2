...
export function PortalSidebar() {
  ...
  const tenantName = portalUser?.tenant?.name || t('login.brandName')
  ...
  return (<>
    <aside className='hidden lg:flex fixed top-0 left-0 z-40 h-full bg-jl-card flex-col w-[260px] border-r border-jl overflow-hidden'>
      <div className='flex items-center gap-3 px-4 h-16 border-b border-jl shrink-0'>
        <img src="/icon.png" alt={t('login.brandName')} className='size-8 rounded-lg shrink-0 object-cover' />
    ...
    <div className='p-4 lg:p-6 space-y-4'>
      ...
      {tab === 'resume' && (
        <div className='space-y-4'>
          ...
          {!caseDetail.notes?.length && !caseDetail.tasks?.length && <EmptyState icon={BookOpen} title={t('portal.noTaskNotes')} description={t('portal.noNotes')} />}
        </div>
      )}
      {tab === 'timeline' && (
        <div className='space-y-2'>
...
export function PortalNotificationsView() {
  ...
  return (
    <div className='p-4 lg:p-6 space-y-4'>
      ...
      {(!notifications || notifications.length === 0) && <EmptyState icon={Bell} title={t('portal.noNotifications')} description={t('portal.noNotifications')} />}
