...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Header
    doc.font('Helvetica-Bold').fontSize(16).fillColor(COLORS.primary).text('Rapport Financier', 50, 40)
    doc.font('Helvetica').fontSize(9).fillColor(COLORS.gray).text(`${tenant?.name || ''} — Généré le ${fmtD(new Date())}`, 50, 60)
    if (startDate || endDate) {
      const range = `Période: ${startDate ? fmtD(startDate) : '...'} — ${endDate ? fmtD(endDate) : '...'}`
      doc.text(range, 50, 74)
    }
    ...
    for (let i = 0; i < kpis.length; i++) {
      const x = 50 + i * (boxW + 8)
      doc.roundedRect(x, y, boxW, 40, 4).stroke(COLORS.border)
      doc.font('Helvetica').fontSize(7).fillColor(COLORS.gray).text(kpis[i].label, x + 8, y + 8, { width: boxW - 16 })
      doc.font('Helvetica-Bold').fontSize(12).fillColor(kpis[i].color).text(kpis[i].value, x + 8, y + 22, { width: boxW - 16 })
    }
    ...
    // Totals
    y += 5
    doc.rect(50, y, 490, 1).fill(COLORS.border)
    y += 8
    ...
    doc.font('Helvetica').fontSize(7).fillColor(COLORS.gray).text('JurisLink — Rapport financier généré automatiquement', 50, 748, { width: 490, align: 'center' })
