...
import { PAYMENT_METHOD_LABELS, PAYMENT_METHOD_COLORS } from '@/lib/constants'
...
import { Download, FileSpreadsheet, FileText, TrendingUp, Banknote, Clock, AlertCircle, AlertTriangle, CreditCard, Wallet } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="size-10 rounded-lg bg-[#E8F0F8] flex items-center justify-center shrink-0"><TrendingUp className="size-5 text-[#1E5A8A]" /></div><div className="min-w-0"><p className="text-xs text-[#6B7280]">Encaissé (filtré)</p><p className="text-base sm:text-lg font-bold text-[#1E5A8A] mt-1 truncate" title={fmtMoney(filteredTotal)}>{fmtMoney(filteredTotal, 'XAF', true)}</p></div></div></CardContent></Card>
