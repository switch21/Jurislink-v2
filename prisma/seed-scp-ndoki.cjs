...
const CLIENTS = [
  ...
  { fullName: 'Aimée Fotso', company: null, clientType: 'particulier', email: 'a.fotso@gmail.com', phone: '+237 6 99 00 06 06', city: 'Douala', country: 'Cameroun', source: 'parrainage', riskLevel: 'faible' },
]
  ...
  for (let i = 0; i < CASES.length; i++) {
    ...
    if (c.status !== 'clos' && c.status !== 'nouveau') {
      ...
      for (const name of c.assignees.slice(0, 2)) {
        await db.$executeRawUnsafe("INSERT INTO event_assignments (id, user_id, event_id) VALUES ('" + crypto.randomUUID() + "', '" + USER_IDS[name] + "', '" + ev1.id + "') ON CONFLICT DO NOTHING")
      }
      if (c.caseType === 'penal') {
        ...
        for (const name of c.assignees.slice(0, 2)) {
          await db.$executeRawUnsafe("INSERT INTO event_assignments (id, user_id, event_id) VALUES ('" + crypto.randomUUID() + "', '" + USER_IDS[name] + "', '" + ev2.id + "') ON CONFLICT DO NOTHING")
        }
      } else {
